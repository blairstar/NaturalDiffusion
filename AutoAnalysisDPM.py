...
def sampling_tx():
    ...
    # skip_alpha_bars = alpha_bars[roi_idxs]
    #
    # skip_alphas = np.zeros_like(skip_alpha_bars)
    # skip_alphas[0] = skip_alpha_bars[0]
    # for ii in range(1, len(skip_alpha_bars)):
    #     skip_alphas[ii] = skip_alpha_bars[ii] / skip_alpha_bars[ii - 1]
    # 
    # skip_betas = 1 - skip_alphas

    step = len(roi_idxs)
    step_idxs = list(range(step))[::-1]
    
    t_start = roi_idxs[step_idxs[0]]
    eps_t = symbols("eps_%03d"%t_start)
    add_item("eps_%03d"%t_start, eps_t)
    add_item("x_%03d"%t_start, eps_t*1.0)
    
    all_time_idxs = [roi_idxs[step_idxs[0]]]
    for ii in step_idxs[:-1]:
        t_idx = roi_idxs[ii]
        next_t_idx = roi_idxs[ii-1]
        
        alpha_bar_curr = alpha_bars[t_idx]
        
        x_curr = get_item("x_%03d"%t_idx)
        
        # first step
        y_name = "y_%03d"%t_idx
        y_curr = symbols(y_name)
        add_item(y_name, y_curr)
        
        score_curr = (np.sqrt(alpha_bar_curr)*y_curr - x_curr)/(1-alpha_bar_curr)
        
        # velo_curr = -0.5*x_curr - 0.5*score_curr
        velo_curr = -0.5*x_curr - 0.5*score_curr
        beta_integral = -1*np.sum(betas[next_t_idx:t_idx+1])
        
        x_next = x_curr + beta_integral * velo_curr

        # second step
        alpha_bar_next = alpha_bars[next_t_idx]

        y_next_name = "y_%03d_next"%next_t_idx
        y_next = symbols(y_next_name)
        add_item(y_next_name, y_next)
        
        score_next = (np.sqrt(alpha_bar_next)*y_next - x_next)/(1-alpha_bar_next)

        velo_next = -0.5 * x_curr - 0.5 * score_next
        
        x_next_new = x_curr + 0.5*beta_integral*(velo_curr + velo_next)

        add_item("x_%03d"%next_t_idx, x_next_new)
        all_time_idxs.append(next_t_idx)
    
    all_time_idxs = sorted(list(set(all_time_idxs)), reverse=True)
    
    ys = get_y_symbols()
    epss = get_eps_symbols()
    
    for t_idx in all_time_idxs:
        x_curr = get_item("x_%03d"%t_idx)
        
        print("t", t_idx)
        y_coeffs = show_symbol_coeff(x_curr, ys)
        true_y_scale = np.sqrt(alpha_bars[t_idx])
        print("y result", np.sum(y_coeffs), true_y_scale)
        
        eps_coeffs = show_symbol_coeff(x_curr, epss)
        true_eps_scale = np.sqrt(1-alpha_bars[t_idx])
        print("eps result", np.linalg.norm(eps_coeffs), true_eps_scale)
        print("")
